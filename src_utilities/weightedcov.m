...
if ~ctrl
  error('Input size(Y, 1) has to be equal to length(w)!')
end

[T, N] = size(Y);                                                                 % T: number of observations; N: number of variables
C = Y - repmat(w' * Y, T, 1);                                                     % Remove mean (which is, also, weighted)
C = C' * (C .* repmat(w, 1, N));                                                  % Weighted Covariance Matrix (biased version)
coeff = sum(w)./((sum(w)).^2-sum(w.^2));
C = coeff.*C;                                                                     % Weighted Covariance Matrix (UNbiased version)
