...
if nin == 1, % only x is provided
    ...
    if m==1,
        warning('MATLAB:cov_gPC:InputDim', 'First input x, must be 1-D vertical vector. x is transposed!');
        x=x'; [m,n] = size(x); poly_norm = ones(m,1); % default polynomial norm taken to 1.0
    end
else % i.e. nin = 2
